...
if (!$conn) {
    ...
    $query = "select hours.status, users.name as 'name_user', works.name, works.code, works.id as 'id_work', hours.date, hours.hour, hours.comment, hours.id, who_approve from hours, users, works where users.id=hours.user_id and hours.work_id=works.id and hours.id=$id;";
    ...
    if(isset($_REQUEST['edit'])){
        ...
        $query4 = "update hours set status='APROBADA', who_approve=(select email from users where id=$id_user ) where id=$id; ";
